...
├── .github/workflows/test.yml   CI: the suite on Node 20 and 22
├── CLAUDE.md               this file
...
└── demo/                   Deliverable 1
    ...
    │   │   ├── sorting.js      view-only sort keys; money sorts on base value (D-16)
...
| D2 — Design document | **Done** — `design/PRODUCTION-DESIGN.md`: 23 sections, a sources appendix, 14 diagrams. Named so that no filename in the repository collides with `demo/Design.md`. Scoped by `design/BRIEF.md`. Every section states the problem, the options, the trade-offs, the chosen solution, its consequences, and what would change the decision; every figure is measured, quoted from documentation, or labelled an estimate. A carry-over audit after completion checked every demo decision and constant against the design: nine defects fixed, two in guarantee statements (the rounding count is per tranche; the resolution gate at zero weight), and §1.5 now records the disposition of each demo decision. A full consistency pass followed — three independent sweeps over structure and cross-references, numbers and constants, and concepts and terminology — and closed every item found: stale wording left by the audit's own edits, a `#sources` anchor collision, the §6.13 guarantee labels renamed NL1–NL7 to stop colliding with assumptions A1–A11, the sources appendix aligned with the body, and the reader guide's links pointed at subsection anchors. The worked-example providers were then chosen against the design's own requirements, from primary documentation and practitioner reports rather than preference: Google Cloud (Cloud SQL Enterprise Plus, Cloud Run, Binary Authorization) and Gemini on Vertex AI, with Claude on the same endpoint as the alternative the evaluation corpus can select; §21 records the trade-offs, including the retirement-cadence evidence that favours the alternative |
